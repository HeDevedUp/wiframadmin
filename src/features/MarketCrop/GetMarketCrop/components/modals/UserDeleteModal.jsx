...
const UserDeleteModal = ({ element, marketdata, onDelete, openNow, onClose }) => {
    ...
    return (
        <Modal element={element} openNow={openNow} onClose={() => onClose()}>
            ...
            >
                <Trash size={22} className="mr-2" /> Deleting Market Data {marketdata.cropName}
            </Dialog.Title>
            <div className="mt-2">
                <p className="text-sm text-darker-t">
                    The item {marketdata.cropName} will be permanently deleted.
                </p>
                <p className="text-sm text-darker-t">
